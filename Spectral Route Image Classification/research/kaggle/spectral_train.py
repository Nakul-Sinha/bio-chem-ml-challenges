...
CFG=dict(MODEL=env("MODEL","convnext_small.fb_in22k"), IMG=int(env("IMG","224")),
         FOLDS=int(env("FOLDS","5")), EPOCHS=int(env("EPOCHS","16")), BS=int(env("BS","32")),
         LR=float(env("LR","2e-4")), WD=float(env("WD","0.05")), SEV=float(env("SEV","0.85")),
         SEED=int(env("SEED","0")), NW=int(env("NW","2")), TAG=env("TAG","cs224"))
