...
- **FINAL = rdkit-free n-gram MLP (3 seeds), OOF composite 0.422, public ~0.388.** Imports only
  numpy/pandas/torch; no rdkit, no subprocess, no internet. solution.py + submission.csv consistent
  (submission regenerated by solution.py). Zero divergence / dependency risk.
