...
def main():
    ...
    vocab=pd.read_csv(DATA/"solvent_vocabulary.csv")
    ...
    def train_one(seed):
        ...
        Xa=torch.tensor(Xtr); yt=torch.tensor(Ytemp); ytm=torch.tensor(Ytime); yc=torch.tensor(Ycat)
        ysv=torch.tensor(Ysolv); yp=torch.tensor(Yprim)
        ...
        with torch.no_grad():
            ...
            for i in range(0,len(Xte),1024):
                ...
                o["temp"].append(torch.softmax(lt,1).cpu().numpy()); o["time"].append(torch.softmax(ltm,1).cpu().numpy())
    ...
    pr=p["prim"].copy(); pr[:,0]=pr[:,0]*NONE_BOOST; pa=pr.argmax(1); solv=[]
    ...
    sub=pd.DataFrame({"reaction_id":test["reaction_id"],
        ...
        "pred_temp_bin":[TEMPS[i] for i in tp],"pred_time_bin":[TIMES[i] for i in tmp],
        "pred_catalyst_present":cat})
